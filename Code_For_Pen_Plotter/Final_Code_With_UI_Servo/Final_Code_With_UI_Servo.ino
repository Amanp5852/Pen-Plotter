...
#define EEPROM_ADDR_SERVO (EEPROM_ADDR_POS + 2 * sizeof(long)) // New EEPROM address for servo position
...
#define SERVO_PIN 26 // New pin definition for the servo motor
...
int currentServoPos = 90; // Default center position
...
void moveServo(int degrees); // New function prototype for servo control
void saveServoPosition(int pos); // New EEPROM function for servo
void loadServoPosition(int& pos); // New EEPROM function for servo
...
void setup() {
    ...
    loadServoPosition(currentServoPos); // Load last known servo position
...
// Save servo position to EEPROM
void saveServoPosition(int pos) {
    EEPROM.put(EEPROM_ADDR_SERVO, pos);
    EEPROM.commit();
    Serial.printf("Saved servo position: %d\n", pos);
}

// Load servo position from EEPROM
void loadServoPosition(int& pos) {
    EEPROM.get(EEPROM_ADDR_SERVO, pos);
    if (pos < 0 || pos > 180) {
        pos = 90; // Default to center if invalid
    }
    Serial.printf("Loaded servo position: %d\n", pos);
}
...
void moveServo(int degrees) {
    // Restrict the servo movement to a safe range to prevent mechanical stress
    degrees = constrain(degrees, 0, 180);
    myServo.write(degrees);
    currentServoPos = degrees;
    saveServoPosition(currentServoPos); // Save the new position to EEPROM
...
<head>
    ...
    <style>
        ...
        .button-group.servo-control {
            display: grid;
            grid-template-columns: 1fr 1fr; /* Two columns for servo control */
            gap: 8px;
            margin-bottom: 10px;
        }
...
<body>
    <div class="main-container">
        ...
        <div class="card">
            <h2>Servo Control</h2>
            <div class="button-group servo-control">
                <button class="button btn-green" onclick="sendServoCommand(0)">Pen Up (0°)</button>
                <button class="button btn-green" onclick="sendServoCommand(90)">Center (90°)</button>
                <button class="button btn-green" onclick="sendServoCommand(180)">Pen Down (180°)</button>
            </div>
            <div class="input-group">
                 <input type="number" id="servo-input" value="5" placeholder="Degrees">
                 <button class="button btn-green" onclick="sendServoCommandRelative()">Move by Degrees</button>
            </div>
        </div>
    ...
    <script>
        ...
        function sendServoCommand(degrees) {
            log(`Sending servo command: Move to ${degrees}°`);
            fetch(`/command?cmd=servo&val=${degrees}`)
                .then(response => response.text())
                .then(data => log(`Response: ${data}`))
                .catch(error => log(`Error: ${error}`));
        }
        
        function sendServoCommandRelative() {
            const degrees = document.getElementById('servo-input').value;
            if (!degrees) {
                log('Please enter a value for the servo degrees.');
                return;
            }
            log(`Sending servo command: Move by ${degrees}°`);
            fetch(`/command?cmd=servo_relative&val=${degrees}`)
                .then(response => response.text())
                .then(data => log(`Response: ${data}`))
                .catch(error => log(`Error: ${error}`));
        }
...
void handleCommand() {
    ...
    if (cmd == "home") {
        ...
        int degrees = val.toInt();
        moveServo(degrees);
    } else if (cmd == "servo_relative") {
        int degrees = val.toInt();
        int newPos = currentServoPos + degrees;
        moveServo(newPos);
    }
